...
本项目为南京理工大学江阴校区 Combat 战队 2026 赛季雷达无线电链路相关代码，覆盖干扰波接收、信息波接收、协议编解码、波形生成、频点参数配置以及与主程序服务器的联动通信。本项目仅需要一块pluto sdr模块可实现对干扰波和信息波的接收，在开启gui_launcher.py时，东部赛区实测稳定解析2次干扰波，但信息波解析率较低（大概率是由于信息波和干扰波功率相差较大而信息波被覆盖，实测比赛开始和结束时都能接收到几帧信息波数据)。安装python依赖后即可部署。
雷达解析信息波作为今年提出的新规，雷达组成员也只是出于摸索阶段，水平有限，旨在开源已经在赛场上实际验证可行的sdr方案，存在诸多不足的地方请大家多多包涵，仅供参考，并提醒sdr使用请遵守国家法律法规，不承担任何他人使用不当造成的不良后果，希望能给各位RMer带来一些帮助！
